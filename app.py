...
def get_tv_symbol(underlying: str, strike: float, option_type: str, expiry_date_str: str) -> str:
    ...
    if u_upper in ("SENSEX", "BSX"):
        exchange = "BSE"
        tv_underlying = "BSX"
    elif u_upper in ("BANKEX", "BKX"):
        exchange = "BSE"
        tv_underlying = "BKX"
    else:
        tv_underlying = u_upper
        
    opt_char = "C" if option_type.upper() in ("CE", "CALL") else "P"
    strike_int = int(strike)
    
    # TradingView Format: BSE:BSX260806C79400 or NSE:NIFTY260804P24300
    return f"{exchange}:{tv_underlying}{year_yy}{month_mm}{day_dd}{opt_char}{strike_int}"
...
def _run_tv_screenshot_sync(tid: int) -> None:
    ...
    try:
        ...
        # Run in-process inside the background thread worker to save memory footprint on 1GB VPS
        from capture_tv import capture_screenshot
        success = capture_screenshot(
            symbol, timeframe, output_path,
            TRADINGVIEW_SESSIONID, TRADINGVIEW_SESSIONID_SIGN, TRADINGVIEW_LAYOUT_ID,
            t["date"], scroll_time
        )
...
def report_page():
    ...
    u   = request.args.get("underlying") or ""
    
    all_rows = []
    with _cache_lock:
        for dt, day_rows in _trades_cache.items():
            if from_date <= dt <= to_date:
                all_rows.extend(day_rows)
    all_rows.sort(key=lambda r: (r["date"], r.get("entry_time", "")))
    rows = [r for r in all_rows if not u or r["underlying"] == u.upper()]
    ...
    for r in rows:
        ...
        # Generate HTML for each tag separately
        tt = r.get("trade_type", "")
        if tt:
            bg = "#e8f5e9" if tt == "hedge" else "#e1f5fe" if tt == "scalp" else "#ffebee"
            fg = "#2e7d32" if tt == "hedge" else "#0277bd" if tt == "scalp" else "#c62828"
            tt_html = f'<span style="background:{bg};color:{fg};padding:2px 6px;border-radius:4px;font-size:9px;font-weight:600;border:1px solid {fg}40;text-transform:uppercase">{tt}</span>'
        else:
            tt_html = '—'
        ...
        if tf:
            ...
            tf_html = f'<span style="background:{bg};color:{fg};padding:2px 6px;border-radius:4px;font-size:9px;font-weight:600;border:1px solid {fg}40">{tf}</span>'
        else:
            tf_html = '—'
        ...
        if rf is not None:
            ...
            lbl = "✓ Yes" if rf == 1 else "✗ No"
            rf_html = f'<span style="background:{bg};color:{fg};padding:2px 6px;border-radius:4px;font-size:9px;font-weight:600;border:1px solid {fg}40">{lbl}</span>'
        else:
            rf_html = '—'
            
        strat = r.get("strategy", "")
        strat_html = f'<span style="font-size:10px;color:#444;font-weight:500;font-style:italic">{strat}</span>' if strat else '—'

        rows_html += f"""<tr>
          <td>{r.get('entry_time','')}</td>
          <td><span style="color:{dir_col}">{direction}</span></td>
          <td><span style="color:{opt_col}">{opt}</span></td>
          <td>{r.get('strike','')}</td>
          <td>{r.get('entry_price','')}</td>
          <td>{r.get('exit_price') or '—'}</td>
          ...
          <td>{tf_html}</td>
          <td>{rf_html}</td>
          <td style="white-space:pre-wrap;word-break:break-word">{strat_html}</td>
          <td style="white-space:pre-wrap;word-break:break-word">{r.get('notes','') or ''}</td>
        </tr>"""

    total_color = "#3fb950" if total_pnl >= 0 else "#f85149"
    total_sign  = "+" if total_pnl >= 0 else ""
    header_u    = f" — {u.upper()}" if u else ""

    html = f"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>Trade Report {date_label}{header_u}</title>
<style>
  body {{ font-family: 'Segoe UI', Arial, sans-serif; font-size: 12px; color: #222;
         margin: 24px 32px; background: #fff; }}
  h1   {{ font-size: 18px; margin-bottom: 4px; }}
  .sub {{ font-size: 11px; color: #666; margin-bottom: 16px; }}
  .summary {{ display:flex; gap:24px; margin-bottom:18px; padding:10px 16px;
              background:#f5f5f5; border-radius:6px; border:1px solid #ddd; }}
  .sm {{ text-align:center; }}
  .sm .v {{ font-size:16px; font-weight:700; }}
  .sm .l {{ font-size:10px; color:#666; text-transform:uppercase; letter-spacing:.5px; }}
  table {{ width:100%; border-collapse:collapse; font-size:11px; }}
  th {{ background:#f0f0f0; border:1px solid #ccc; padding:6px 8px; text-align:left;
       font-size:10px; text-transform:uppercase; letter-spacing:.4px; color:#444;
       cursor:pointer; user-select:none; transition: background 0.1s; }}
  th:hover {{ background:#e2e2e2; }}
  td {{ border:1px solid #e0e0e0; padding:6px 8px; vertical-align:top; }}
  tr:nth-child(even) td {{ background:#fafafa; }}
  .footer {{ margin-top:18px; font-size:10px; color:#888; border-top:1px solid #eee;
             padding-top:8px; }}
  @media print {{
    body {{ margin: 12px; }}
...
<div class="no-print" style="margin-top:10px; margin-bottom:14px; background:#f9f9f9; padding:12px; border-radius:6px; border:1px solid #ddd; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
  ...
  <div style="display:flex; flex-wrap:wrap; gap:8px;">
    <div style="display:flex; flex-direction:column; gap:2px;">
      <label style="font-size:10px; color:#888; font-weight:600;">From Date</label>
      <input type="date" id="f-from-date" value="{from_date}" onchange="changeReportRange()" style="font-size:11px; padding:4px 8px; border-radius:4px; border:1px solid #ccc; outline:none; background:#fff; cursor:pointer; font-weight:500;">
    </div>
    <div style="display:flex; flex-direction:column; gap:2px;">
      <label style="font-size:10px; color:#888; font-weight:600;">To Date</label>
      <input type="date" id="f-to-date" value="{to_date}" onchange="changeReportRange()" style="font-size:11px; padding:4px 8px; border-radius:4px; border:1px solid #ccc; outline:none; background:#fff; cursor:pointer; font-weight:500;">
    </div>
    <div style="display:flex; flex-direction:column; gap:2px;">
...
<div class="sub" id="sumSub">{date_label} &nbsp;&#8226;&nbsp; {len(rows)} trades &nbsp;&#8226;&nbsp; {len(closed)} closed</div>
<div class="summary">
...
<table>
  <thead><tr>
    ...
    <th>Lots</th><th>P&amp;L</th><th>Trade Type</th><th>TF</th><th>Rules</th><th>Strategy</th><th>Notes</th>
  </tr></thead>
  <tbody>{rows_html}</tbody>
</table>
<div class="footer">Generated by Trade Analyser {APP_VERSION} &nbsp;&#8226;&nbsp; {date_label}</div>

<script>
var _root = '{root}';
function changeReportRange() {{
  const fromVal = document.getElementById('f-from-date').value;
  const toVal = document.getElementById('f-to-date').value;
  if (!fromVal || !toVal) return;
  window.location.href = _root + '/report?from_date=' + fromVal + '&to_date=' + toVal;
}}
...
document.querySelectorAll('th').forEach((th, idx) => {{
  ...
  th.addEventListener('click', () => {{
    ...
    document.querySelectorAll('th').forEach(h => h.dataset.asc = '');
    ...
    rows.sort((a, b) => {{
      ...
      const cleanNum = (str) => {{
        // Strip out currency symbol, commas, and trailing L for lots
        let s = str.replace(/[₹,]/g, '').replace(/L$/, '').trim();
        if (s === '—' || s === '--' || !s) return asc ? Infinity : -Infinity;
        const val = parseFloat(s);
        return isNaN(val) ? (asc ? Infinity : -Infinity) : val;
      }};
      
      const isNumCol = [3, 4, 5, 7, 8].includes(idx); // Strike, Entry Rs, Exit Rs, Lots, P&L
      if (isNumCol) {{
        return asc ? cleanNum(valA) - cleanNum(valB) : cleanNum(valB) - cleanNum(valA);
      }}
...
function filterRows() {{
  ...
  const fromVal = document.getElementById('f-from-date').value;
  const toVal = document.getElementById('f-to-date').value;
  const rangeStr = (fromVal === toVal) ? fromVal : (fromVal + ' to ' + toVal);
  document.getElementById('sumSub').innerHTML = 
    `${{rangeStr}} &nbsp;&#8226;&nbsp; ${{totalRows}} trades &nbsp;&#8226;&nbsp; ${{closedCount}} closed`;
}}
</script>
</body>
</html>"""
...
.hbtn.on {{ background: #1a2a1a; border-color: #3a6a3a; color: #4fc3f7 }}
...
.pane-btn {{ position:absolute; right:78px; z-index:10; background:rgba(20,20,20,0.75);
...
.tpill.tp-scalp   {{ background:rgba(79,195,247,.15);  color:#4fc3f7; border-color:#4fc3f7; }}
.tpill.tp-short   {{ background:rgba(239,83,80,.15);   color:#ef5350; border-color:#ef5350; }}
    ...
    <div id="pbody">
      ...
      <table id="tbl" style="display:none">
        <thead><tr>
          ...
          <th style="width:64px">Entry &#8377;</th><th style="width:64px">Exit &#8377;</th><th style="width:52px">Lots</th><th style="width:70px">P&amp;L</th><th>Notes</th><th style="width:58px"></th>
        </tr></thead>
        <tbody id="tbody"></tbody>
      </table>
    </div>
